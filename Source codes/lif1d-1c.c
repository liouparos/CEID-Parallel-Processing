...
int main(int argc, char *argv[])
{
	...
	double		s_min;
	double		s_max;
	double		*sum1, *u, *uplus, *sigma, *omega, *omega1, *sum, *p1;
	double		sum2, sumdot;
	double		time;
  ...
  sum1 = (double *)calloc(n, sizeof(double));
	if (sum1 == NULL) {
		printf("Could not allocate memory for \"sum1\".\n");
		exit(1);
	}
   ...
   gettimeofday(&global_start, NULL);
 	for (it = 0; it < itime; it++) {
      ...
      sumdot = 0.0;
      p1 = sigma + i*n;
      sumdot = cblas_ddot(n, p1, 1, u, 1);      
 			uplus[i] = u[i] + dt * (mu - u[i]) + dt * (sumdot - sum2) / divide;
 		}
